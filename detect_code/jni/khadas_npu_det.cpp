...
JNI_OnLoad(JavaVM* vm, void* reserved __unused)
{
    ...
    if (registerNativeMethods(env,"com/khadas/npudemo/Inceptionv3Manager", sMethods, NELEM(sMethods)) < 0)
    return -1;

    gJavaVM = vm;

    clazz = (*env).FindClass("com/khadas/npudemo/Inceptionv3Manager");
    if (clazz == NULL) {
        log_error("FindClass com/khadas/npudemo/Inceptionv3Manager failed");
        return -1;
    }
